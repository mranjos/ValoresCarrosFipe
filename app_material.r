...
ui <- material_page(
  title = "Histórico de valores de veículos FIPE",
  tags$br(),
  material_row(
    material_column(
      width = 3,
      material_card(
        title = "",
        depth = 4,
        tags$p("Data de Referência"),
        selectInput("data_referencia",
                    "",
                    choices = format(as.Date(Sys.Date()),format="%b/%Y")),
        tags$p("Marca do Veículo"),
        uiOutput("secondSelection"),
        tags$p("Modelo do Veículo"),
        uiOutput("terceiroSelection"),
        tags$p("Ano do Modelo do Veículo"),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$data_referencia,{
    ...
    output$secondSelection <- renderUI({
      selectInput("marca", "", choices = c(marcas_referencia$make_name))
    })
  })
  ...
  observeEvent(input$marca,{
    ...
    output$terceiroSelection <- renderUI({
      selectInput("modelo", "", choices = c(modelos_referencia$model_name))
    })
  })
  ...
  observeEvent(input$modelo,{
    ...
    output$quartoSelection <- renderUI({
      selectInput("ano", "", choices = c(anos_referencia$year))
    })
  })
